...
float axis_unit = 10;
int16_t RTS::update_time_value = 0;
...
void RTS::RTS_USBPrint_Set() {
    ...
    rts.sendData("USB Print", PRINT_FILE_TEXT_VP);
    rts.sendData(1, Time_VP);
    rts.sendData(ExchangePageBase + 11, ExchangepageAddr);
}

void RTS::RTS_USBPrint_Finish() {
    if (card.isPrinting()) {
        return;
    }
    if (print_state == 0) {
    ...
    rts.sendData(0, PRINT_FILE_TEXT_VP);
    rts.sendData(ExchangePageBase + 1, ExchangepageAddr);
}

void RTS::RTS_Set_Waitway(const char new_waitway) {
    waitway = new_waitway;
}
